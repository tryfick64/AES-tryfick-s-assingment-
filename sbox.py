from funcs import g_mult


def affine_transform(byte):
    c = 0x63  # Константа 01100011
    result = 0
    for i in range(8):
        bit = (  # то есть мы в начале сдвигаем до нужного бита а потом & с 1 чтобы получить этот бит
                (byte >> i & 1) ^
                (byte >> ((i + 4) % 8) & 1) ^
                (byte >> ((i + 5) % 8) & 1) ^
                (byte >> ((i + 6) % 8) & 1) ^
                (byte >> ((i + 7) % 8) & 1) ^
                (c >> i & 1)
        )  # формула: bi = Bi xor B(i+4)mod8 xor B(i+5)mod8 xor B(i+6)mod8 xor B(i+7)mod8 xor Ci
        result |= (bit << i)
    return result
...
def gen_sbox():
    ...
    sbox = [g_mult_inv(byte) for byte in sbox]
    sbox = [affine_transform(byte) for byte in sbox]
    return sbox


def gen_inv_sbox(sbox):
    inv_sbox = [0] * 256
...
def print_box(box):
    ...
    for i in range(0, 256, 16):
        print("    " + ", ".join(f"0x{byte:X}" for byte in box[i:i + 16]) + ",")
    print("]")


if __name__ == '__main__':
    print_box(gen_sbox())
    print_box(gen_inv_sbox(gen_sbox()))
